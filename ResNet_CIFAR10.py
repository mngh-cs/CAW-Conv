...
def get_cifar10_dataloader(root, train_batch_size=128, test_batch_size=128, seed=2222):
    ...
    train_dataset, valid_dataset = torch.utils.data.random_split(train_dataset, [49000, 1000])
...
class Resnet(nn.Module):
    ...
    def update(self, dataloader):
        ...
        #print("loss of layers (Mean):", temp_dict)
    ...
    def pruning(self, dataloader, test_dataloader, k=5):
        ...
        for i in range(len(self.layers)):
            if loss_layers_dict[str(i)]:
                avg_loss = torch.tensor(loss_layers_dict[str(i)], dtype=torch.float).item()
            else:
                avg_loss = float('inf')
        ...
        #print("losses_tensor:", losses_tensor)
        weights = torch.softmax(-losses_tensor, dim=0)  # negative because lower loss is better
        #print(f"[Pruning] Layer weights (from loss): {weights.tolist()}")
        ...
        return valid_acc, before_val_acc, test_acc, before_test_acc  # Return before_test_acc
    ...
    def save_layer_weights(self, filename="all_epochs_layer_weights.txt", epoch=None):
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if epoch%10 == 0:
            model.save_layer_weights(weight_filename, epoch=epoch)
        ...
        valid_acc, before_acc, test_acc, before_test_acc = model.pruning(valid_loader, test_loader, k=5)  # Pass test_loader
        ...
        with open(info_csv_path, mode='a', newline='') as f:
            ...
            writer.writerow([
                ...
                round(before_acc, 2),
                round(valid_acc, 2),
                round(before_test_acc, 2),  # Add this
                round(test_acc, 2),
                round(train_time, 2),
        ...
        if (epoch + 1) % 25 == 0:
            ...
            print(f"Epoch {(epoch + 1):03d}: Maximum Test Accuracy Prunning: {max_test_acc_pruning:.2f}%")
            print("==================")
