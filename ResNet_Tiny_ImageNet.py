...
class CWConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=False, num_class=200, dropout=0, reg_ortho=1e-3,
                 reg_entropy=1e-2, temperature=10, temp_decay_rate=0.995):
        super().__init__()

        assert out_channels % num_class == 0, f"out_channels ({out_channels}) must be divisible by num_class ({num_class})"
...
def prepare_tiny_imagenet_val(val_dir, val_annotations_file):
    """Preprocess Tiny ImageNet validation set into class folders."""
    val_dir = Path(val_dir)
    annotations_file = val_dir / val_annotations_file
    ...
    val_data = pd.read_csv(annotations_file, sep='\t', header=None,
                           names=['filename', 'class_id', '_', '_', '_', '_'])

    for class_id in val_data['class_id'].unique():
    ...
    for _, row in val_data.iterrows():
        ...
        dst = val_dir / row['class_id'] / row['filename']
...
def get_tinyimagenet_dataloader(root, train_batch_size=128, test_batch_size=128, seed=2222):
    root = Path(root)
    train_dir = root / 'train'
    val_dir = root / 'val'

    if not root.exists():
        raise FileNotFoundError(f"Tiny ImageNet dataset root directory not found: {root}")
    if not train_dir.exists():
        raise FileNotFoundError(f"Tiny ImageNet training directory not found: {train_dir}")
    if not val_dir.exists():
        raise FileNotFoundError(f"Tiny ImageNet validation directory not found: {val_dir}")

    # Preprocess validation set if not already organized
    val_annotations = val_dir / 'val_annotations.txt'
    if val_annotations.exists() and (val_dir / 'images').exists():
        print(f"Preprocessing Tiny ImageNet validation set at {val_dir}")
        prepare_tiny_imagenet_val(val_dir, 'val_annotations.txt')

    # Verify class folders
    train_classes = len([d for d in train_dir.iterdir() if d.is_dir()])
    val_classes = len([d for d in val_dir.iterdir() if d.is_dir() and d.name != 'images'])
    if train_classes != 200:
        raise ValueError(f"Expected 200 classes in train directory, found {train_classes}")
    if val_classes != 200:
        raise ValueError(f"Expected 200 classes in val directory, found {val_classes}")
    ...
    val_dataset = datasets.ImageFolder(str(val_dir), transform=transform_val)

    torch.manual_seed(seed)
    # Use a subset for validation from train (10%)
...
class Resnet(nn.Module):
    def __init__(self,
        ...
        self.layers = nn.ModuleList([
            ...
            CWConv(planes[0]*2, planes[1], 3, stride=2, padding=1, bias=bias, num_class=num_class, dropout=dropout, reg_ortho=1e-3), #5
            ...
            CWConv(planes[1]*2, planes[2], 3, stride=2, padding=1, bias=bias, num_class=num_class, dropout=dropout, reg_ortho=1e-3), #9
            ...
            CWConv(planes[2]*2, planes[3], 3, stride=2, padding=1, bias=bias, num_class=num_class, dropout=dropout, reg_ortho=1e-3),  #13
    ...
    def pruning(self, dataloader, test_dataloader, k=5):
        ...
        before_test_acc = 100.0 * full_test_correct / total_test
    ...
    def save_layer_weights(self, filename="all_epochs_layer_weights.csv", epoch=None, stats_filename="layer_weight_stats.csv"):
        ...
        headers = ["epoch", "layer", "class_idx", "out_channels"]
        for ch in range(max([layer.out_channels for layer in self.layers])):
            headers.append(f"weight_ch{ch}")
        
        append_mode = 'a' if os.path.exists(filename) and epoch != 0 else 'w'
        with open(filename, append_mode, newline='') as csvfile:
            ...
            for i, layer in enumerate(self.layers):
                if hasattr(layer, 'current_weights') and layer.current_weights is not None:
                    weights = layer.current_weights  # (out_channels, num_class)
                    for class_idx in range(min(weights.shape[1], self.num_class)):
                        row = [epoch, i, class_idx, weights.shape[0]]
                        class_weights = weights[:, class_idx].numpy().round(4)
                        # Pad with zeros if fewer channels than max
                        padded_weights = np.pad(class_weights, (0, max(0, max([layer.out_channels for layer in self.layers]) - len(class_weights))), 'constant')
                        row.extend(padded_weights.tolist())
                        writer.writerow(row)
                else:
                    row = [epoch, i, -1, 0] + [0.0] * max([layer.out_channels for layer in self.layers])
                    writer.writerow(row)
        ...
        with open(stats_filename, append_mode, newline='') as csvfile:
            ...
            for i, layer in enumerate(self.layers):
                ...
                # Conv weights stats
                conv_w = layer.conv.weight.detach().cpu()
                row.extend([conv_w.mean().item(), conv_w.std().item(), 
                            conv_w.min().item(), conv_w.max().item()])
                
                # Raw class_channel_weights stats (overall)
                raw_ch = layer.class_channel_weights.detach().cpu()
                row.extend([raw_ch.mean().item(), raw_ch.std().item()])
                
                # Softmaxed current_weights stats (overall)
                if layer.current_weights is not None:
                    soft_ch = layer.current_weights
                    ...
                    # Average sparsity: fraction of weights < 0.01 per class
    ...
    def compute_pairwise_classification_stats(self, dataloader):
        devices = [next(layer.parameters()).device for layer in self.layers]
        self.eval()
        num_layers = len(self.layers)
        cw_matrix = torch.zeros(num_layers, num_layers)
        wc_matrix = torch.zeros(num_layers, num_layers)
        cc_matrix = torch.zeros(num_layers, num_layers)
        ww_matrix = torch.zeros(num_layers, num_layers)
        with torch.no_grad():
            for x, labels in dataloader:
                shortcut = torch.zeros(1)
                x, labels = x.to(devices[0]), labels.to(devices[0])
                ...
                for i, layer in enumerate(self.layers):
                    ...
                    if self.input_shortcut_flag[i]:
                        shortcut = F.avg_pool2d(x, 2, stride=2).detach() if self.downsample_flag[i] else x.detach()
                layer_preds = torch.stack(layer_preds, dim=1)  # bs x num_layers
                labels_cpu = labels.cpu()
                correct_matrix = (layer_preds == labels_cpu.unsqueeze(1))  # bs x num_layers
                for i in range(num_layers):
                    for j in range(num_layers):
                        cw_matrix[i, j] += (correct_matrix[:, i] & ~correct_matrix[:, j]).sum().item()
                        wc_matrix[i, j] += (~correct_matrix[:, i] & correct_matrix[:, j]).sum().item()
                        cc_matrix[i, j] += (correct_matrix[:, i] & correct_matrix[:, j]).sum().item()
                        ww_matrix[i, j] += (~correct_matrix[:, i] & ~correct_matrix[:, j]).sum().item()
        return cw_matrix, wc_matrix, cc_matrix, ww_matrix
...
if __name__ == '__main__':
    ...
    args = argparse.Namespace(
        ...
        save_step=5,
        queue_size=5,
        data_dir='/vol/hd2/users/navid/analysis3/data/tiny-imagenet-200'
    )
    ...
    in_channels = 3
    num_class = 200  # Tiny ImageNet has 200 classes
    start_epoch = 0
    ...
    if task == 'tiny-imagenet':
        train_loader, valid_loader, test_loader = get_tinyimagenet_dataloader(root=root, train_batch_size=train_batch_size,
                                                                              test_batch_size=test_batch_size, seed=seed)

    model = None
    if args.arch == 'resnet':
        model = Resnet(in_channels=in_channels, num_class=num_class, dropout=dropout,
                       planes=[200, 400, 800, 1600], learning_rate=lr, lr_min=lr_min, weight_decay=weight_decay,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch + 1) % 50 == 0:
            cw_matrix, wc_matrix, cc_matrix, ww_matrix = model.compute_pairwise_classification_stats(valid_loader)
            pd.DataFrame(cw_matrix.numpy(), columns=[f"Layer_{j}" for j in range(len(model.layers))]).to_csv(
                os.path.join(task_dir, f'pairwise_correct_wrong_epoch{epoch+1}.csv'), index_label='Layer'
            )
            pd.DataFrame(wc_matrix.numpy(), columns=[f"Layer_{j}" for j in range(len(model.layers))]).to_csv(
                os.path.join(task_dir, f'pairwise_wrong_correct_epoch{epoch+1}.csv'), index_label='Layer'
            )
            pd.DataFrame(cc_matrix.numpy(), columns=[f"Layer_{j}" for j in range(len(model.layers))]).to_csv(
                os.path.join(task_dir, f'pairwise_correct_correct_epoch{epoch+1}.csv'), index_label='Layer'
            )
            pd.DataFrame(ww_matrix.numpy(), columns=[f"Layer_{j}" for j in range(len(model.layers))]).to_csv(
                os.path.join(task_dir, f'pairwise_wrong_wrong_epoch{epoch+1}.csv'), index_label='Layer'
            )
