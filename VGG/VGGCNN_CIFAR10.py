...
filename_postfix = "vgg"  # Updated to reflect VGG architecture
...
with open(ortho_csv_path, mode='w', newline='') as f:
    writer = csv.writer(f)
    header = ["epoch"] + [f"layer_{i}_ortho_loss" for i in range(13)] + [f"layer_{i}_ortho_score" for i in range(13)]
...
loss_layers = []
loss_layers_dict = {f'{i}':[] for i in range(13)}
current_layer = 0
current_epoch = 0
...
with open(csv_file, mode='w', newline='') as file:
    ...
    header = [f'Layer_{i}' for i in range(0, 13)]
    writer.writerow(header)

class Vgg(nn.Module):
    def __init__(self, in_channels=3, num_class=10, dropout=0., bias=False, learning_rate=0.03, weight_decay=0., devices=None, epochs=150, lr_decay=0.1):
    ...
    def pruning(self, dataloader, test_dataloader, k=5):
        ...
        weights = torch.softmax(-losses_tensor, dim=0)
...
if __name__ == '__main__':
    ...
    model = None
    if args.arch == 'vgg':
        model = Vgg(in_channels=in_channels, num_class=num_class, dropout=dropout,
